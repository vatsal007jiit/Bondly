...
const SideMenu = () => {
  ...
  const [dp,setDp] = useState(user.gender==='Male'?avatar:avatarFem)
    ...
    <div>
      ...
      <nav className="text-lg space-y-4">
        <div className="flex items-center gap-1 mt-2 p-1  ">  
          <img src={dp} alt="Avatar" className="h-16 w-16 rounded-full" />
        ...
        <div onClick={()=>logout(navigate, setSession)} className="hover:text-indigo-500 dark:hover:text-indigo-400 transition cursor-pointer hover:bg-gray-200 hover:dark:bg-gray-700 p-1 mb-2 rounded-lg">🚪 Logout</div>
      </nav>
